...
export class NotificationJob {
    public static async runNotificationJob(bot: SOEBot, timestamp: number, res: any, sendDebugData: boolean): Promise<number> {
        ...
        for (let i = 0; i < questions.length; i++) {
            ...
            if (i !== 0 && i % maxMsgsToConvoInOneSec === 0) {
                await NotificationJob.sleep(delayInMilliseconds);
            }
            ...
            */
            ...
            // if stringOfChanges stayed an empty string, then do not send updates
            // if stringOfChanges has an entry, then need to send updates
            if (stringOfChanges) {
    ...
    public static async respondWithError(res: any): Promise<void> {
        res.send(`<html>
            <body>
            <p>
